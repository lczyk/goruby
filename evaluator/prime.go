...
// available regardless of whether `require` was called (mri loads
// it lazily; we install eagerly for simplicity).
...
// [[-1,1], ...] for n<0 (sign carried as a -1 factor). We follow that.
func primeDivision(env *object.Environment, args []object.RubyObject) (object.RubyObject, error) {
	if len(args) != 1 {
		return nil, errorf("evaluator: Prime.prime_division: wrong number of arguments (given %d, expected 1)", len(args))
	}
	intArg, ok := args[0].(*object.Integer)
	if !ok {
		return nil, errorf("evaluator: Prime.prime_division: expected Integer, got %T", args[0])
	}
	n := intArg.ToBig()
	if n.Sign() == 0 {
	...
	// Trial-divide by 2, then odd primes 3, 5, 7, ...
	exp := int64(0)
	two := big.NewInt(2)
...
// [[prime, exponent], ...] pairs. Mirrors MRI's
...
func primeIntFromPrimeDivision(env *object.Environment, args []object.RubyObject) (object.RubyObject, error) {
	if len(args) != 1 {
		return nil, errorf("evaluator: Prime.int_from_prime_division: wrong number of arguments (given %d, expected 1)", len(args))
	}
	arr, ok := args[0].(*object.Array)
	if !ok {
		return nil, errorf("evaluator: Prime.int_from_prime_division: expected Array, got %T", args[0])
	}
	out := big.NewInt(1)
	for i, e := range arr.Elements {
		pair, ok := e.(*object.Array)
		if !ok || len(pair.Elements) != 2 {
			return nil, errorf("evaluator: Prime.int_from_prime_division: element %d not a [prime, exponent] pair", i)
		}
		p, okP := pair.Elements[0].(*object.Integer)
		expI, okE := pair.Elements[1].(*object.Integer)
		if !okP || !okE {
			return nil, errorf("evaluator: Prime.int_from_prime_division: pair %d must be Integers", i)
		}
		if expI.Value < 0 {
			return nil, errorf("evaluator: Prime.int_from_prime_division: negative exponent")
		}
